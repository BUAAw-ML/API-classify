...
class GCNBert(nn.Module):
    ...
    def forward(self, ids, token_type_ids, attention_mask, inputs_tfidf, encoded_tag, tag_mask, tag_embedding_file,
        ...
        # # pred = torch.sum(attention_out, -1) + x
        # pred = x * torch.sigmoid(self.weight4)
    ...
    def get_config_optim(self, lr, lrp):
        return [
                ...
                {'params': self.bert.encoder.layer[11].parameters(), 'lr': 0.06},
                {'params': self.bert.encoder.layer[10].parameters(), 'lr': 0.04},
                {'params': self.bert.encoder.layer[9].parameters(), 'lr': 0.02},
                {'params': self.bert.encoder.layer[8].parameters(), 'lr': 0.01},
                {'params': self.bert.encoder.layer[7].parameters(), 'lr': 0.005},
                {'params': self.bert.encoder.layer[6].parameters(), 'lr': 0.001},
