...
class GCNBert(nn.Module):
    ...
    def forward(self, ids, token_type_ids, attention_mask, inputs_tfidf, encoded_tag, tag_mask, tag_embedding_file,
        ...
        token_feat = token_feat.transpose(1, 2).unsqueeze(1).repeat(1,self.num_classes,1,1)
        tag_words_embedding = tag_words_embedding.unsqueeze(0).repeat(8, 1, 1, 1)

        attention = torch.mutmul(tag_words_embedding, token_feat) #N, num_classes, 7, seq_len
