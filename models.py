...
class GCNBert(nn.Module):
    ...
    def forward(self, ids, token_type_ids, attention_mask, inputs_tfidf, encoded_tag, tag_mask, tag_embedding_file, tfidf_result):
        ...
        # weight1 = torch.sigmoid(self.weight1(attention_out)).squeeze(-1)
        # weight2 = torch.sigmoid(self.weight2(x)).squeeze(-1).unsqueeze(0)
        ...
        pred = attention_out + (self.res - 1).unsqueeze(0) * x
